...
int main()
{
	...
	SACM_MS02_Play(MIDI_Index, DAC1, Ramp_Up + Ramp_Dn);	// playback with auto mode
	while(1)
	{
		Key = SP_GetCh();
		switch(Key)
		{	
			...
			case 0x0100:	// IOA0 + Vcc
				SACM_MS02_Play(MIDI_Index, DAC1, Ramp_Up + Ramp_Dn);		// play speech
				break;

			case 0x0200:	// IOA1 + Vcc
				SACM_MS02_Stop();						// stop
				break;

			case 0x0400:	// IOA2 + Vcc
				PauseFlag ^= 0x0001;
				if(PauseFlag)
