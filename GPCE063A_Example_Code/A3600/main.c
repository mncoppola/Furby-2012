//==========================================================================
// SACM-A3600 algorithm with play, stop, pause, resume, volume up, volume down function
//
// Function call for SACM-A3600:
// Syntax:
//  void SACM_A3600_Initial(void)
//
//  void SACM_A3600_Play(int Speech_Index, int DA_Channel, int Ramp_Set)
...
//
// void SACM_A3600_Stop(void);
// 
// void SACM_A3600_Pause(void);
// void SACM_A3600_Resume(void);
//
// void SACM_A3600_Volume(unsigned VolumeValue);
//      VolumeValue: 0x0000 ~ 0xFFFF
//
// int SACM_A3600_Status(void);
//      bit0: 0: Idle
//            1: Audio active
//
// Note:
//   1. User would need to check out the resource.asm for resource arrangement.
//      The speech table is T_SACM_A3600_SpeechTable for A3600.
//   2. For manual mode, user have to handle the memory address and data
//      acquisition through F_USER_A3600_GetData, _USER_A3600_SetStartAddr
//   3. The RAM taken by library can be shared by declaring ORAM section,
//      "OVERLAP_A3600_RAM_BLOCK:   .Section        .ORAM".
...
int main()
{
	...
	SPI_Initial();
	SACM_A3600_Initial();		// A3600 initial
	SACM_A3600_Play(SpeechIndex, DAC1 + DAC2, Ramp_Up + Ramp_Dn);	// playback with auto mode
	while(1)
	{
		Key = SP_GetCh();
		switch(Key)
		{	
			...
			case 0x0001:	// IOA0 + Vcc
				PlayCon = 0;
				SACM_A3600_Play(SpeechIndex, DAC1 + DAC2, Ramp_Up + Ramp_Dn);		// play speech
				break;

			case 0x0002:	// IOA1 + Vcc
				PlayCon = 0;
				SACM_A3600_Stop();						// stop
				break;

			case 0x0004:	// IOA2 + Vcc
				SACM_A3600_Pause();						// playback pause
				break;

			case 0x0008:	// IOA3 + Vcc
				SACM_A3600_Resume();					// playback resuem
				break;
			...
			case 0x0010:	// IOA4 + Vcc
				if(++SpeechIndex >= MaxSpeechNum)		// next speech
					SpeechIndex = 0;
				PlayCon = 0;
				SACM_A3600_Play(SpeechIndex, DAC1 + DAC2, Ramp_Up + Ramp_Dn);		// play next speech
				break;

			case 0x0020:	// IOA5 + Vcc
				if(++VolumeIndex >= MaxVolumeNum)
					VolumeIndex = 0;
				USER_A3600_Volume(VolumeIndex);			// volume up
				break;

			case 0x0040:	// IOA6 + Vcc
				if(++DAC_FIR_Type > C_DAC_FIR_Type3)
					DAC_FIR_Type = C_DAC_FIR_Type0;
				SACM_A3600_DA_FIRType(DAC_FIR_Type);	// change DAC filter type
				break;

			case 0x0080:	// IOA7 + Vcc
				PlayCon = 1;
				SpeechIndex = 0;
				USER_A3600_SetStartAddr(SpeechIndex);
				SACM_A3600_Play(Manual_Mode_Index, DAC1 + DAC2, Ramp_Up);
				break;
		...
		if(PlayCon)
		{
			if(SACM_A3600_Check_Con() == -1)
			{
				if(++SpeechIndex >= MaxSpeechNum)		// next speech
					SpeechIndex = 0;
				USER_A3600_SetStartAddr_Con(SpeechIndex);
				SACM_A3600_Play_Con(Manual_Mode_Index, DAC1 + DAC2, Ramp_Dn);
			}
			SACM_A3600_ServiceLoop();
			WatchdogClear();
		}
