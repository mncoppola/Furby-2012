//==========================================================================
// SACM-S530 algorithm with play, stop, pause, resume, volume up, volume down function
//
// Function call for SACM-S530:
// Syntax:
//  void SACM_S530_Initial(void)
//
//  void SACM_S530_Play(int Speech_Index, int DA_Channel, int Ramp_Set)
...
//
// void SACM_S530_Stop(void);
// 
// void SACM_S530_Pause(void);
// void SACM_S530_Resume(void);
//
// void SACM_S530_Volume(unsigned VolumeValue);
//      VolumeValue: 0x0000 ~ 0xFFFF
//
// int SACM_S530_Status(void);
//      bit0: 0: Idle
//            1: Audio active
//
// Note:
//   1. User would need to check out the resource.asm for resource arrangement.
//      The speech table is T_SACM_S530_SpeechTable for S530.
//   2. For manual mode, user have to handle the memory address and data
//      acquisition through F_USER_S530_GetData, _USER_S530_SetStartAddr
//   3. The RAM taken by library can be shared by declaring ORAM section,
//      "OVERLAP_S530_RAM_BLOCK:   .Section        .ORAM".
...
int main()
{
	...
	SPI_Initial();
	SACM_S530_Initial();		// S530 initial
	SACM_S530_Play(SpeechIndex, DAC1 + DAC2, Ramp_Up + Ramp_Dn);	// playback with auto mode
	while(1)
	{
		Key = SP_GetCh();
		switch(Key)
		{	
			...
			case 0x0001:	// IOA0 + Vcc
				PlayCon = 0;
				SACM_S530_Play(SpeechIndex, DAC1 + DAC2, Ramp_Up + Ramp_Dn);		// play speech
				break;
			...
			case 0x0004:	// IOA2 + Vcc
				SACM_S530_Pause();						// playback pause
				break;

			case 0x0008:	// IOA3 + Vcc
				SACM_S530_Resume();					// playback resuem
				break;
			...
			case 0x0010:	// IOA4 + Vcc
				if(++SpeechIndex >= MaxSpeechNum)			// next speech
					SpeechIndex = 0;
				PlayCon = 0;
				SACM_S530_Play(SpeechIndex, DAC1 + DAC2, Ramp_Up + Ramp_Dn);		// play next speech
				break;

			case 0x0020:	// IOA5 + Vcc
				if(++VolumeIndex >= MaxVolumeNum)
					VolumeIndex = 0;
				USER_S530_Volume(VolumeIndex);			// volume up
				break;

			case 0x0040:	// IOA6 + Vcc
				if(++DAC_FIR_Type > C_DAC_FIR_Type3)
					DAC_FIR_Type = C_DAC_FIR_Type0;
				SACM_S530_DA_FIRType(DAC_FIR_Type);		// change DAC filter type
				break;

			case 0x0080:	// IOA7 + Vcc
				PlayCon = 1;
				SpeechIndex = 0;
				USER_S530_SetStartAddr(SpeechIndex);
				SACM_S530_Play(Manual_Mode_Index, DAC1 + DAC2, Ramp_Up);
				break;
		...
		if(PlayCon)
		{
			if(SACM_S530_Check_Con() == -1)
			{
				if(++SpeechIndex >= MaxSpeechNum)		// next speech
					SpeechIndex = 0;
				USER_S530_SetStartAddr_Con(SpeechIndex);
				SACM_S530_Play_Con(Manual_Mode_Index, DAC1 + DAC2, Ramp_Dn);
			}
		}
		if(ServiceType == Foreground)
			SACM_S530_ServiceLoop();
		System_ServiceLoop();
	} // end of while
   	return 0;	
}	