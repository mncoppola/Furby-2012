...
int main()
{
	...
	SPI_Initial();
	SACM_A1601_Initial();		// A1601 initial
	SACM_A1601_Play(SpeechIndex, DAC1 + DAC2, Ramp_Up + Ramp_Dn);	// playback with auto mode
	while(1)
	{
		Key = SP_GetCh();
		switch(Key)
		{	
			...
			case 0x0001:	// IOA0 + Vcc
				PlayCon = 0;
				SACM_A1601_Play(SpeechIndex, DAC1 + DAC2, Ramp_Up + Ramp_Dn);		// play speech
				break;

			case 0x0002:	// IOA1 + Vcc
				PlayCon = 0;
				SACM_A1601_Stop();						// stop
				break;

			case 0x0004:	// IOA2 + Vcc
				SACM_A1601_Pause();						// playback pause
				break;

			case 0x0008:	// IOA3 + Vcc
				SACM_A1601_Resume();					// playback resuem
				break;
			...
			case 0x0010:	// IOA4 + Vcc
				if(++SpeechIndex >= MaxSpeechNum)		// next speech
					SpeechIndex = 0;
				PlayCon = 0;
				SACM_A1601_Play(SpeechIndex, DAC1 + DAC2, Ramp_Up + Ramp_Dn);		// play next speech
				break;

			case 0x0020:	// IOA5 + Vcc
				if(++VolumeIndex >= MaxVolumeNum)
					VolumeIndex = 0;
				USER_A1601_Volume(VolumeIndex);			// volume up
				break;

			case 0x0040:	// IOA6 + Vcc
				if(++DAC_FIR_Type > C_DAC_FIR_Type3)
					DAC_FIR_Type = C_DAC_FIR_Type0;
				SACM_A1601_DA_FIRType(DAC_FIR_Type);	// change DAC filter type
				break;

			case 0x0080:	// IOA7 + Vcc
				PlayCon = 1;
				SpeechIndex = 0;
				USER_A1601_SetStartAddr(SpeechIndex);
				SACM_A1601_Play(Manual_Mode_Index, DAC1 + DAC2, Ramp_Up);
				break;
		...
		if(PlayCon)
		{
			if(SACM_A1601_Check_Con() == -1)
			{
				if(++SpeechIndex >= MaxSpeechNum)		// next speech
					SpeechIndex = 0;
				USER_A1601_SetStartAddr_Con(SpeechIndex);
				SACM_A1601_Play_Con(Manual_Mode_Index, DAC1 + DAC2, Ramp_Dn);
			}
		}
